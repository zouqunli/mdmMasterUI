...
/**
 * @author mdm
 * @desc  贝塞尔曲线图 todo 未开始写暂时搁置
 * @create 2021/12/15
 */
public class MdmLineBezierChart extends View {
    ...
    public MdmLineBezierChart(Context context) {
        this(context,null);
    }
    public MdmLineBezierChart(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }
    public MdmLineBezierChart(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        canvas.save();
        ...
        for(int i = 0;i < mData.size();i++){
            float x = originRectF.left + i * cellWidth + dxCellWidth;
            float y = originRectF.bottom - mData.get(i) * cellHeight;
            if(i == 0){
                //折线不是封闭图形所以不需要从原点开始，而是从第一个高度点开始
                mPath.moveTo(x,y);
                mBgPath.lineTo(x,y);
            }else{
                set3BezierControl(x - cellWidth,originRectF.bottom - mData.get(i-1) * cellHeight,x,y);
                mPath.cubicTo(controlPoint1.x,controlPoint1.y,controlPoint2.x,controlPoint2.y,x,y);
                mBgPath.cubicTo(controlPoint1.x,controlPoint1.y,controlPoint2.x,controlPoint2.y,x,y);
            }
            //记录背景路径

            //画节点
            if(isShowCircle)
            canvas.drawCircle(x,y,circleRadius, mPointPaint);
            //画节点值
            drawValue(canvas,mData.get(i),x,y);
        }
        //最后需要画个垂线到x轴上
...
//        canvas.restore();
    ...
    //算出每两个点的两个控制点坐标
    private void set3BezierControl(float x1,float y1,float x2,float y2){
        if(controlPoint1 == null)controlPoint1 = new PointF();
        if(controlPoint2 == null)controlPoint2 = new PointF();
        controlPoint1.x = x1 + a*(x2 - x1);
        controlPoint1.y = y1;
        controlPoint2.x = x1 + a*(x2 - x1);
        controlPoint2.y = y2;
    }

    private void startAnimation(){
//        ObjectAnimator.ofFloat(this, String.valueOf(progress),0,1).start();
    }

    //设置数据
    public MdmLineBezierChart setData(List<Float> list){
    ...
    //添加数据
    public MdmLineBezierChart addData(float data){
...
//        startAnimation();
    ...
    //清除数据
    public MdmLineBezierChart clearData(){
        if(mData != null)mData.clear();
        return this;
    }

    //设置显示列表的个数
    public MdmLineBezierChart setPointLength(int length){
        if(length < 0){
            length = 10;
        }
        mPointLength = length;
    ...
    //是否自动取最大值
    public MdmLineBezierChart isAutoMaxValue(boolean isAutoMaxVaule){
        this.isAutoMaxValue = isAutoMaxVaule;
        return this;
    }

    //是否画节点，并配置节点的半径
    public MdmLineBezierChart isShowCircle(boolean isShowCircle, float radius){
    ...
    //设置背景颜色
    public MdmLineBezierChart setCanvasBg(int color){
    ...
    //设置坐标轴颜色
    public MdmLineBezierChart setAxisColor(int color){
        if(color == 0)return this;
    ...
    //设置折线颜色
    public MdmLineBezierChart setLineColor(int color){
        if(color == 0)return this;
    ...
    //设置折线底部背景颜色
    public MdmLineBezierChart setLineRegionColor(int[] regionColors){
